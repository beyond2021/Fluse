...
//  Fluse
//
//  Created by KEEVIN MITCHELL on 5/17/24.
//
...
class FormViewModel {
    ...
    var name = ""
    var amount: Double = 0
    var category:Category = Category.utilities
    var date = Date()
    // Disable if the form is empty
    var isSavedButtonDisabled: Bool {
        name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    }
    // Clousure for currency
    ...
    //when its nil we are adding a new log else we are editing
